...
void compare(Int_t evt
      , Int_t chan1=1, Int_t chan2=2, Int_t chan3=0, Int_t chan4=0
      , Int_t chan5=0, Int_t chan6=0, Int_t chan7=0, Int_t chan8=0
      , Int_t chan9=0, Int_t chan10=0, Int_t chan11=0, Int_t chan12=0
      , TTree* tree=0, const char* wname="compare")
{
   //cout<< "compare" <<endl;
   ...
   if (newformat) {
      ...
      if (chan9 > 0 and tree->GetLeaf("c9")) channels[nchannels++] = chan9;
      if (chan10 > 0 and tree->GetLeaf("c10")) channels[nchannels++] = chan10;
      if (chan11 > 0 and tree->GetLeaf("c11")) channels[nchannels++] = chan11;
      if (chan12 > 0 and tree->GetLeaf("c12")) channels[nchannels++] = chan12;
   }
   else {
      ...
      if (chan9 > 0 and tree->GetLeaf("b3_c1")) channels[nchannels++] = chan9;
      if (chan10 > 0 and tree->GetLeaf("b3_c2")) channels[nchannels++] = chan10;
      if (chan11 > 0 and tree->GetLeaf("b3_c3")) channels[nchannels++] = chan11;
      if (chan12 > 0 and tree->GetLeaf("b3_c4")) channels[nchannels++] = chan12;
   }
   ...
   for (int ix=0; ix<ndivx; ++ix)
   {
      ...
      {
         //cout<< "ix + ndivx*iy + 1 = " << ix + ndivx*iy + 1 <<endl;
         can->cd(ix + ndivx*iy + 1);

         Int_t ich = ix*ndivy + iy;
         if (ich < nchannels) {
            Int_t board = (channels[ich]-1)/4 + 1;
            //cout<< "ich = " << ich << " newformat = " << newformat <<endl;
            //cout<< "var expression = " << Form("c%d:t%d",channels[ich],board) << " tree = " << tree << " tree->GetName() = " << tree->GetName() <<endl;
            std::string varexp;
            if (newformat) varexp = Form("c%d:t%d",channels[ich],board);
            else {
               Int_t channel = (channels[ich]-1) % 4 + 1;      // old channels from 1 to 4
               varexp = Form("b%d_c%d:b%d_t", board,channel,board);
            }
            TCut cut = Form("Entry$==%d",evt);
            //cout<< "--> before the Draw: varexp.c_str() = " << varexp.c_str() << " cut.GetTitle() = " << cut.GetTitle() <<endl;
            tree->Draw(varexp.c_str(), cut, "lp");
            //cout<< "--> after the Draw" <<endl;
      ...
      , TTree* tree=0, const char* wname="compare")
{
   ...
   while (true)
   {
      ...
      {
         ++evtNo;
         compare(evtNo,chan1,chan2,chan3,chan4,chan5,chan6,chan7,chan8,chan9,chan10,chan11,chan12,tree,wname);
         continue;
      }

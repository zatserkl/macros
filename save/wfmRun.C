...
namespace Tree {
   Float_t t[4][100000], v[4][100000];    // buffer for the tree

   Float_t pmax[4];           // maximum of the smoothed pulse
   Float_t xmax[4];           // position of the maximum
   Float_t time[4];           // time of the half-maximum in the smoothed pulse
   Float_t timeraw[4];        // time of the half-maximum in the raw data
   Float_t trise[4];          // rise time (smoothed)
   Float_t triseraw[4];       // rise time (raw)
   Float_t bkg[4];            // baseline (raw data)
   ...
   void book(TTree* tree) {
      tree->Branch("pmax", &pmax, "pmax[4]/F");
      tree->Branch("xmax", &xmax, "xmax[4]/F");
      tree->Branch("time", &time, "time[4]/F");
      tree->Branch("timeraw", &timeraw, "timeraw[4]/F");
      tree->Branch("trise", &trise, "trise[4]/F");
      tree->Branch("triseraw", &triseraw, "triseraw[4]/F");
      tree->Branch("bkg", &bkg, "bkg[4]/F");
   }

   void connect(TTree* tree) {
      tree->SetBranchAddress("t1", &t[0]);
      tree->SetBranchAddress("v1", &v[0]);
      tree->SetBranchAddress("t2", &t[1]);
      tree->SetBranchAddress("v2", &v[1]);
      tree->SetBranchAddress("t3", &t[2]);
      tree->SetBranchAddress("v3", &v[2]);
      tree->SetBranchAddress("t4", &t[3]);
      tree->SetBranchAddress("v4", &v[3]);
   }
}
...
TGraph* gmoving_average(Int_t naver, TGraph* g=0, bool plot=true, bool same=true) {
   if (!g) g = gtemp();
   if (!g) {
      cout<< "Could not find graph in the current canvas" <<endl;
      return 0;
   }

   Double_t* ya = new Double_t[g->GetN()];

   moving_average(naver, g->GetN(), g->GetY(), ya);

   TGraph* ga = new TGraph(g->GetN(), g->GetX(), ya);
   ga->SetNameTitle(Form("%s_av_%d",g->GetName(),naver), Form("%s averaged %d points",g->GetTitle(),naver));
   ga->SetMarkerStyle(g->GetMarkerStyle());
   Int_t color = 2;
   if (g->GetMarkerColor() == 2) color = 4;
   ga->SetMarkerColor(color);
   ga->SetLineColor(color);
...
void timeres(Int_t naver=65, const char* ifname="Run11-dia.root"
             , Float_t xbkg=-50
             , Float_t scale0=-1.
             , Float_t scale1=-1.
             , Float_t scale2=1.
             ...
             )
{
   ...
   TFile* ofile = new TFile(Form("%s.res-%d.root",ifname,naver), "recreate");
   TTree* tree = (TTree*) ifile->Get("wfm");

   Int_t np = tree->GetLeaf("t0")->GetLen();

   Tree::connect(tree);

   TTree* otree = new TTree("res", Form("analysis of %s",ifname));
   ...
   if (naver % 2 == 0) {
      naver++;
      cout<< "changed naver to odd number " << naver <<endl;
   }
   ...
   for (int ientry=0; ientry<tree->GetEntries(); ++ientry)
   {
      ...
      for (int ich=0; ich<4; ++ich) for (int i=0; i<np; ++i) Tree::v[ich][i] *= scale[ich];
      ...
      for (int ich=0; ich<4; ++ich) {
         Int_t nbkg = 0;
         for (int i=0; i<np; ++i) {
            if (Tree::t[ich][i] > xbkg) break;
            Tree::bkg[ich] += Tree::v[ich][i];
            ++nbkg;
         }
         Tree::bkg[ich] /= nbkg;
         for (int i=0; i<np; ++i) Tree::v[ich][i] -= Tree::bkg[ich];    // subtract baseline for this channel
      }

      for (int ich=0; ich<4; ++ich) {
         moving_average(naver, np, Tree::v[ich], va[ich]);
      }

      // find the maximum in the smoothed distribution

      for (int ich=0; ich<4; ++ich) {
         Int_t imax = 0;
         for (int i=0; i<np; ++i) {
            if (va[ich][i] > va[ich][imax]) imax = i;
         }
         Tree::xmax[ich] = Tree::t[ich][imax];
         Tree::pmax[ich] = Tree::v[ich][imax];

         // time at averaged half-maximum. NB: the imax is index of maxmum of the averaged spectra

         Float_t halfmax = Tree::pmax[ich]/2;

         for (int i=imax; i>=0; --i) {
            if (va[ich][i] < halfmax) break;
            Tree::time[ich] = Tree::t[ich][i];
         }

         for (int i=imax; i>=0; --i) {
            if (Tree::v[ich][i] < halfmax) break;
            Tree::timeraw[ich] = Tree::t[ich][i];
         }

         // rise time

         Float_t v12 = 0.12*Tree::pmax[ich];
         Float_t v88 = 0.88*Tree::pmax[ich];
         Int_t i12 = 0;
         Int_t i88 = 0;
         for (int i=imax; i>0; --i) {
            if (!i88 && va[ich][i] >= v88 && va[ich][i-1] < v88) i88 = i;
            if (!i12 && va[ich][i] >= v12 && va[ich][i-1] < v12) {
               i12 = i;
               break;
            }
         }
         Tree::trise[ich] = Tree::t[ich][i88] - Tree::t[ich][i12];

         i12 = 0;
         i88 = 0;
         for (int i=imax; i>0; --i) {
            if (!i88 && Tree::v[ich][i] >= v88 && Tree::v[ich][i-1] < v88) i88 = i;
            if (!i12 && Tree::v[ich][i] >= v12 && Tree::v[ich][i-1] < v12) {
               i12 = i;
               break;
            }
         }
         Tree::triseraw[ich] = Tree::t[ich][i88] - Tree::t[ich][i12];
      }

      otree->Fill();
   }

   cout<< "Write " << tree->GetEntry() << " events into output file " << ofile->GetName() <<endl;
   ofile->Write();
}

void mag_average(Double_t* x, Double_t* y, Int_t N, Double_t xmin, Double_t xmax)
{
   TVirtualFFT* fft = TVirtualFFT::FFT(1, &N, "R2C ES K");     // forward transform

   Int_t nsets = 0;
   Double_t mag[20000];
   for (int i=0; i<N/2+1; ++i) mag[i] = 0;
   ...
   Int_t curr = s1;
   while (x[N+curr-1] < xmax) {
      fft->SetPoints(&y[curr]);
      fft->Transform();
   ...
   Double_t L = x[N+s1-1] - x[s1];
   cout<< "nsets = " << nsets << " L = " << L <<endl;

   for (int i=0; i<N/2+1; ++i) mag[i] /= nsets;

   TH1D* hmag = new TH1D("hmag", Form("mag average over %d samples",nsets), N/2+1, 0, (N/2+1)/L);
   for (int i=0; i<N/2+1; ++i) hmag->SetBinContent(i+1, mag[i]/TMath::Sqrt(N));
   new TCanvas;
   hmag->Draw();
}

void mag_file(Int_t chan=2, Int_t N=4096, const char* fname="Run11-dia.root")
{
   ...
   Int_t nsets = 0;
   for (int ientry=0; ientry<wfm->GetEntries(); ++ientry)
   {
   ...
   Double_t L = t[N-1] - t[0];

   TH1D* hmag = new TH1D("hmag", Form("mag average over %d samples channel %d",nsets,chan), N/2+1, 0, (N/2+1)/L);
   for (int i=0; i<N/2+1; ++i) hmag->SetBinContent(i+1, mag[i]/TMath::Sqrt(N));
   new TCanvas;
   hmag->Draw();
...
void filter_event(Int_t event, Int_t chan=2, Float_t sign=-1., Double_t fcut1=0.15, Double_t fcut2=10, Int_t N=0)
{
   TTree* wfm = (TTree*) gDirectory->Get("wfm");
   if (!wfm) {
      cout<< "Could not find tree 'wfm'" <<endl;
   ...
   g->SetNameTitle(Form("evt_%d_chan_%d",event,chan), Form("evt %d chan %d",event,chan));
   ...
   Double_t L = t[N-1] - t[0];
   Int_t smin = fcut1*L;             // convert from real world frequency to frequency sample number
   Int_t smax = fcut2*L;
   ...
   TGraph* gout = new TGraph(npoints, t, output);
   gout->SetNameTitle(Form("evt_%d_chan_%d_f1_%0.2f_f2_%0.2f",event,chan,fcut1,fcut2), Form("evt %d chan %d f1=%0.2f f2=%0.2f",event,chan,fcut1,fcut2));
...
void filter_band_graph(Double_t fcut1=0.1, Double_t fcut2=10, TGraph* g=0)
{
   if (!g) g = gtemp();
   ...
   gout->SetNameTitle(Form("%s_f1_%0.2f_f2_%0.2f",g->GetName(),fcut1,fcut2), Form("%s f1=%0.2f f2=%0.2f",g->GetName(),fcut1,fcut2));
   gout->SetMarkerColor(46);
   gout->SetLineColor(46);
...
void shift_graph(Double_t x1=-10, Double_t x2=10, TGraph* g=0)  // fcut in GHz
{
   if (!g) g = gtemp();
   ...
   for (int i=0; i<g->GetN(); ++i) {
      if (g->GetX()[i] < x1) ++ix1;
      if (g->GetX()[i] > x2) break;
      ++ix2;
   }
   Int_t N = ix1 + 1;
   if (N % 2 != 0) N = ix1 < g->GetN()-1? N+1: N-1;

   Int_t offset = ix2 - N + 1;
   ...
   for (int isample=0; isample<N/2+1; ++isample) {
      mag1[isample] = TMath::Sqrt(re[isample]*re[isample] + im[isample]*im[isample]);
   }

   // get the signal part

   Double_t mag2[20000];
   Double_t phase2[20000];

   fft->SetPoints(&g->GetY()[offset]);
   fft->Transform();
   fft->GetPointsComplex(re, im);

   for (int isample=0; isample<N/2+1; ++isample) {
      mag2[isample] = TMath::Sqrt(re[isample]*re[isample] + im[isample]*im[isample]);
      phase2[isample] = TMath::ATan2(im[isample],re[isample]);
   }

   // subtract

   for (int isample=0; isample<N/2+1; ++isample) {
      mag2[isample] -= mag1[isample];
   }

   // create re and im

   for (int isample=0; isample<N/2+1; ++isample) {
      re[isample] = mag2[isample]*TMath::Cos(phase2[isample]);
      im[isample] = mag2[isample]*TMath::Sin(phase2[isample]);
   ...
   TGraph* gout = new TGraph(N, &g->GetX()[offset], output);
   gout->SetNameTitle(Form("%s_subtract",g->GetName()), Form("%s subtract",g->GetTitle()));
   gout->SetMarkerColor(46);
   gout->SetLineColor(46);
...
namespace TreeRes {
   ...
   void connect(TTree* tree) {
      ...
      tree->Branch("pmax",          &pmax);
      tree->Branch("bkg_raw",          &bkg_raw);
      tree->Branch("ebkg_raw",          &ebkg_raw);
      tree->Branch("bkg",          &bkg);
      tree->Branch("ebkg",          &ebkg);
      tree->Branch("xm",          &xm);
      tree->Branch("ym",          &ym);
      tree->Branch("x5mV",          &x5mV);
      tree->Branch("x10mV",          &x10mV);
      tree->Branch("tx",          &tx);
   }
}  // namespace TreeRes
...
void channel3_RC2(Int_t event, Int_t chan=3, Double_t thres=40., Float_t sign=1., Int_t N=0)
{
   TTree* wfm = (TTree*) gDirectory->Get("wfm");
   if (!wfm) {
      cout<< "Could not find tree 'wfm'" <<endl;

                 ...
                 , bool debug=false)
{
    ...
    Double_t charge = 0;
    for (int i=signal_i1; i<=signal_i2; ++i) {
        if (i > halfmax_i2 && w[i] < 0) break;              // stop at the negaitve channel if any
        charge += w[i];
    }
    ...
    // gout->GetFunction("gaus")->SetLineColor(9);
...
void channel_fit_AFP_poli(Int_t N, Double_t* t, Double_t* w
                 , Double_t thres
                 , Double_t trig
                 , Double_t cutoff_GHz
                 , Double_t yfit1=0.30
                 ...
                 , bool debug=false)
{
    if (debug) cout<< "channel_fit_AFP event = " << event << " channel = " << chan <<endl;
    ...
    //--srd-- Int_t signal_i1 = maximum_i - 3*(maximum_i - halfmax_i1);
    Int_t signal_i1 = maximum_i - 2.5*(maximum_i - halfmax_i1);
    if (signal_i1 < 0) signal_i1 = 0;
    //--std-- Int_t signal_i2 = maximum_i + 3*(halfmax_i2 - maximum_i);
    Int_t signal_i2 = maximum_i + 2.*(halfmax_i2 - maximum_i);
    if (signal_i2 > N-1) signal_i2 = N-1;
    Double_t charge = 0;
    for (int i=signal_i1; i<=signal_i2; ++i) {
        if (i > halfmax_i2 && w[i] < 0) break;              // stop at the negaitve channel if any
...
void channel_fit_AFP_RC1gb(Int_t N, Double_t* t, Double_t* w
                 , Double_t thres
                 , Double_t trig
                 , Double_t cutoff_GHz
                 , Double_t yfit1=0.30
                 ...
                 , bool debug=false)
{
    ...
    /// // Double_t x0 = t[i12];
    /// // Double_t A = w[maximum_i]*(tau/2.)*TMath::Exp(2);
    Double_t tau = 0.5*fwhm;
    Double_t x0 = t[maximum_i] - fwhm;
    Double_t A = w[maximum_i]*(tau/2.)*TMath::Exp(2);
    ...
    Int_t offset_low = 8;
    Int_t offset_high = 4;
    Int_t fit_x1_i = maximum_i - offset_low;
    if (fit_x1_i < 0) fit_x1_i = 0;
    fit_x1 = t[fit_x1_i] - eps;
    Int_t fit_x2_i = maximum_i + offset_high;
    if (fit_x2_i > N-1) fit_x2_i = N - 1;
    fit_x2 = t[fit_x2_i] + eps;
    static TF1* f;
    if (!f) f = FunRC::RC1gb(fit_x1,fit_x2, A,x0,tau,sigma);        // NB: RC1gb
    ...
    //  Fit straight line fit first (will be shown on the plot) then gaus
    //
    //--poli-- if (debug) gout->Fit("pol1","","",fit_x1,fit_x2);
    //--poli-- else gout->Fit("pol1","q","goff",fit_x1,fit_x2);
    ...
    Double_t delta = 0.1;
    //--poli-- TF1* fun = gout->GetFunction("pol1");
...
void channel_fit_AFP(Int_t N, Double_t* t, Double_t* w
                 , Double_t thres
                 , Double_t trig
                 , Double_t cutoff_GHz
                 , Double_t yfit1=0.30
                 ...
                 , bool debug=false)
{
    ...
    // f->FixParameter(2, 0.441);
    // f->FixParameter(3, 0.3);
...
void channel_3_fit_AFP(Int_t N, Double_t* t, Double_t* w
                 , Double_t thres
                 , Double_t trig
                 , Double_t cutoff_GHz
                 , Double_t yfit1=0.30
...
void channel1_fit_AFP(Int_t N, Double_t* t, Double_t* w, Double_t thres, Double_t trig, Double_t cutoff_GHz, Double_t yfit1, Double_t yfit2, Int_t chan=1, Int_t event=-1, bool debug=false)
{
    if (debug) cout<< "+++++ channel1_fit_AFP event = " << event << " +++++" <<endl;
    if (debug) cout<< "thres = " << thres << " trig = " << trig << " cutoff_GHz = " << cutoff_GHz << " yfit1 = " << yfit1 << " yfit2 = " << yfit2 << " chan = " << chan << " event = " << event << " debug = " << debug <<endl;
    channel_fit_AFP(N,t,w,thres,trig,cutoff_GHz,yfit1,yfit2,chan,event,debug);
}

void channel2_fit_AFP(Int_t N, Double_t* t, Double_t* w, Double_t thres, Double_t trig, Double_t cutoff_GHz, Double_t yfit1, Double_t yfit2, Int_t chan=2, Int_t event=-1, bool debug=false)
{
    if (debug) cout<< "+++++ channel2_fit_AFP event = " << event << " +++++" <<endl;
    if (debug) cout<< "thres = " << thres << " trig = " << trig << " cutoff_GHz = " << cutoff_GHz << " yfit1 = " << yfit1 << " yfit2 = " << yfit2 << " chan = " << chan << " event = " << event << " debug = " << debug <<endl;
    channel_fit_AFP(N,t,w,thres,trig,cutoff_GHz,yfit1,yfit2,chan,event,debug);
}

void channel3_fit_AFP(Int_t N, Double_t* t, Double_t* w, Double_t thres, Double_t trig, Double_t cutoff_GHz, Double_t yfit1, Double_t yfit2, Int_t chan=3, Int_t event=-1, bool debug=false)
{
    if (debug) cout<< "+++++ channel3_fit_AFP event = " << event << " +++++" <<endl;
    if (debug) cout<< "thres = " << thres << " trig = " << trig << " cutoff_GHz = " << cutoff_GHz << " yfit1 = " << yfit1 << " yfit2 = " << yfit2 << " chan = " << chan << " event = " << event << " debug = " << debug <<endl;
    //-- channel_fit_AFP(N,t,w,thres,trig,cutoff_GHz,yfit1,yfit2,chan,event,debug);
    channel_3_fit_AFP(N,t,w,thres,trig,cutoff_GHz,yfit1,yfit2,chan,event,debug);
}

void channel4_fit_AFP(Int_t N, Double_t* t, Double_t* w, Double_t thres, Double_t trig, Double_t cutoff_GHz, Double_t yfit1, Double_t yfit2, Int_t chan=4, Int_t event=-1, bool debug=false)
{
    if (debug) cout<< "+++++ channel4_fit_AFP event = " << event << " +++++" <<endl;
    if (debug) cout<< "thres = " << thres << " trig = " << trig << " cutoff_GHz = " << cutoff_GHz << " yfit1 = " << yfit1 << " yfit2 = " << yfit2 << " chan = " << chan << " event = " << event << " debug = " << debug <<endl;
    channel_fit_AFP(N,t,w,thres,trig,cutoff_GHz,yfit1,yfit2,chan,event,debug);
}
...
void channel234_run(const char* ifname, Int_t event1=0, Int_t event2=-1, bool debug=false)
{
    ...
    for (int ientry=event1; ientry<=event2; ++ientry)
    {
        ...
        for (int ich=0; ich<nchan; ++ich)
        {
            ...
            if (ich == 2) for (int i=0; i<N; ++i) w[ich][i] = -w[ich][i];   // invert SiPM data
            channel_fit[ich](N, t[ich], w[ich], thres[ich], trig[ich], cutoff_GHz[ich], yfit1[ich],yfit2[ich], chan[ich], ientry, debug);
        }
...
void channel123_run_AFP_pos(const char* ifname, Int_t event1=0, Int_t event2=-1, bool debug=false)
{
    TFile* ifile = new TFile(ifname);
    if (!ifile) {
        cout<< "Could not find file " << ifname <<endl;
    ...
    void (*channel_fit[4])(Int_t N, Double_t* t, Double_t* w, Double_t thres, Double_t trig, Double_t cutoff_GHz, Double_t yfit1, Double_t yfit2, Int_t chan, Int_t event, bool debug);
    channel_fit[0] = channel1_fit_AFP;
    channel_fit[1] = channel2_fit_AFP;
    channel_fit[2] = channel3_fit_AFP;

...
public:
   ...
   TGraphErrors* Plot(Int_t nRC=0, Double_t sigma=-1., Option_t* opt="ap") const
   {
      ...
      for (int i=1; i<gr->GetN(); ++i) {
         if (TMath::Abs(gr->GetX()[i] - x0) < delta) i_x0 = i;
         if (TMath::Abs(gr->GetX()[i] - halfmax1) < delta) i_halfmax1 = i;
         if (TMath::Abs(gr->GetX()[i] - halfmax2) < delta) i_halfmax2 = i;
         if (TMath::Abs(gr->GetX()[i] - pmaxx) < delta) i_pmax = i;
      }
      TMarker* marker_x0 = new TMarker(gr->GetX()[i_x0], gr->GetY()[i_x0], 20);
      marker_x0->SetMarkerColor(1);
      marker_x0->Draw();
      TMarker* marker_halfmax1 = new TMarker(gr->GetX()[i_halfmax1], gr->GetY()[i_halfmax1], 20);
      marker_halfmax1->SetMarkerColor(4);
      marker_halfmax1->Draw();
      TMarker* marker_halfmax2 = new TMarker(gr->GetX()[i_halfmax2], gr->GetY()[i_halfmax2], 20);
      marker_halfmax2->SetMarkerColor(9);
      marker_halfmax2->Draw();
      TMarker* marker_pmax = new TMarker(gr->GetX()[i_pmax], gr->GetY()[i_pmax], 20);
...
public:
   ...
   Int_t evt;
   Int_t np;               //!
   Double_t* x;            //!   pointer to input data (see e.g. SetEventData)
   Double_t* y;            //!   pointer to input data
   TClonesArray* peaks;    //->
...
public:
   void clear() {
      ...
      np = 0;
      peaks->Clear("C");
      for (int ipar=0; ipar<3; ++ipar) par[0] = 0;
   }
   void Clear(Option_t*) {
      // cout<< "PeakChannel::Clear(Option_t*)" <<endl;
      clear();
   }
   PeakChannel(): TObject()
        , chan(0)
   ...
   void SetEventData(Int_t np_, Double_t* x_, Double_t* y_, Int_t evt_, Int_t chan_) {
      evt = evt_;
      chan = chan_;
      np = np_;
      x = x_;
   ...
   bool find_peak(Int_t i0, const Double_t* ys, Peak& peak, Int_t& iend)
   {
      ...
      Int_t iminimum = 0;                    // current min value after the maximum
      Int_t nabove_halfmax = 0;              // the number of points on the tail above 0.5 maximum
      Int_t nabove_halfmax_threshold = 3;    // stop current pulse and set iend to iminimum
      ...
      for (int i=ithres+1; i<np; ++i)
      {
         if (ys[i] > ys[imaximum]) {
            imaximum = i;
            //cout<< "imaximum = " << imaximum << " x[imaximum] = " << x[imaximum] << " ys[imaximum] = " << ys[imaximum] <<endl;
         ...
         if (iminimum == 0) iminimum = i;
         else if (ys[i] < ys[iminimum]) {
            iminimum = i;
            //cout<< "iminimum: x[" << iminimum << "] = " << x[iminimum] << " ys[" << iminimum << "] = " << ys[iminimum] <<endl;
         }

         if (ys[i] < 0) {
            iend = i;
            //cout<< "ys[" << iend << "] < 0: iend = " << iend << " x[iend] = " << x[iend] <<endl;
            break;
         }
         ...
         if (below_threshold_low) {
            if (ys[i] - ys[ibelow_threshold_low] > 0.5*sbkg) {   // beginning of the next peak
               ...
               //cout<< "ys[" << i << "] - ys[" << ibelow_threshold_low << "] > 0.5*sbkg iend = " << iend << " x[iend] = " << x[iend] <<endl;
               break;
            }
         }

         if (iminimum > 0
               && ys[iminimum] < 0.25*ys[imaximum]
               && ys[i] > 0.5*ys[imaximum]
            ) {
            nabove_halfmax++;
            //cout<< "nabove_halfmax = " << nabove_halfmax << " x[" << i << "] = " << x[i] <<endl;
         }
         if (nabove_halfmax >= nabove_halfmax_threshold) {
            iend = iminimum;
            if (debug) cout<< "found beginning of the next peak at x[i] = " << x[i] <<endl;
            break;
         }
      ...
      // steep jump of the the peak (DRS4 problem): ketek75-noamp-nolf-27.0V-100Hz_1.dat.root peakchan(p,1621,1621,true)
      //cout<< "PeakChannel::find_peak: istart = " << istart << " ithres = " << ithres << " iend = " << iend << " imaximum = " << imaximum << " ihalfmax1 = " << ihalfmax1 << " ihalfmax2 = " << ihalfmax2 <<endl;
      //cout<< "PeakChannel::find_peak: x[istart] = " << x[istart] << " x[ithres] = " << x[ithres] << " x[iend] = " << x[iend] << " x[imaximum] = " << x[imaximum] << " x[ihalfmax1] = " << x[ihalfmax1] << " x[ihalfmax2] = " << x[ihalfmax2] <<endl;
      //cout<< "PeakChannel::find_peak: ys[istart] = " << ys[istart] << " ys[ithres] = " << ys[ithres] << " ys[iend] = " << ys[iend] << " ys[imaximum] = " << ys[imaximum] << " ys[ihalfmax1] = " << ys[ihalfmax1] << " ys[ihalfmax2] = " << ys[ihalfmax2] <<endl;
      ...
      if (halfmax_n == 0) {
         cout<< "evt = " << evt << " chan = " << chan << ": halfmax_n == 0" <<endl;
         return false;
      }
      peak.nsigma = halfmax_sum / (sbkg*TMath::Sqrt(halfmax_n));
      //cout<< "peak.nsigma = " << peak.nsigma <<endl;
      ...
      if (true
         ...
         if (debug) {
            cout<< "::find_peak: x[istart] = " << x[istart] << " x[iend] = " << x[iend]
               ...
               << " peak.nsigma = " << peak.nsigma
            <<endl;
            //cout<< "fwhm_min = " << fwhm_min <<endl;
         }
         return true;
      }
      else {
   ...
   void ProcessEvent(Int_t np_, Double_t* x_, Double_t* y_, Int_t evt_)
   {
      //cout<< "PeakChannel::ProcessEvent" <<endl;

      SetEventData(np_, x_, y_, evt_, chan);
      ...
      if (debug) {
         ...
         gr->SetNameTitle(Form("gr_%d",evt), Form("evt %d chan %d",evt,chan));
         gr->SetMarkerStyle(6);
         gr->SetMarkerColor(2);
         gr->SetLineColor(2);
...
public:
   void clear() {
      //cout<< "PeakEvent::clear: pc = " << pc <<endl;
      //cout<< "PeakEvent::clear: pc->GetLast()+1 = " << pc->GetLast()+1 <<endl;
      for (int islot=0; islot<pc->GetLast()+1; ++islot) {
         // PeakChannel* peakChannel = (PeakChannel*) peakEvent->pc->At(slots[ichan]);
         PeakChannel* peakChannel = (PeakChannel*) pc->At(islot);
         peakChannel->Clear("C");
      }
   }
   PeakEvent(): TObject() {
   ...
   PeakEvent(const char* cfg, bool debug_=false): TObject(), debug(debug_)
   {
      ...
      int chan;
      double fwhm = 0;
      for (int ichan=0; ichan<9; ++ichan) fwhm_min[ichan] = 0;
      std::string fwhm_str;
      int iline = 0;
      std::string line;
      ...
      for (int ichan=0; ichan<8; ++ichan) {
         if (fwhm_min[ichan+1] == 0) {
            cout<< "Set to default channel " << ichan+1 <<endl;
            fwhm_min[ichan+1] = fwhm_min[0];
         }
      }

      for (int ichan=0; ichan<8; ++ichan) {
         cout<< "fwhm_min[" << ichan+1 << "] = " << fwhm_min[ichan+1] <<endl;
      }
   }

   ClassDef(PeakEvent, 4);
};
...
void peakchan(TTree* itree
      , Int_t ientry1=0
      , Int_t ientry2=-1
      , bool debug=false
      , const char* cfgname="peakchan.cfg"
      )
{
   ...
   if ((b = itree->GetBranch("t1"))) {
      ...
      for (int ichan=0; ichan<4; ++ichan) {
         if ((b = itree->GetBranch(Form("c%d",ichan+1)))) {
            ...
            //..peakEvent->peakChannel[ichan] = new PeakChannel(ichan+1, peakEvent->fwhm_min[ichan+1], debug);
            new ((*peakEvent->pc)[slots[ichan]]) PeakChannel(ichan+1, peakEvent->fwhm_min[ichan+1], debug);
         }
      }
   }
   ...
   for (int ientry=ientry1; ientry<=ientry2; ++ientry)
   {
      ...
      for (unsigned islot=0; islot<slots.size(); ++islot)
      {
         ...
         if (debug) cout<< "--- evt " << ientry << "   channel " << ichan+1 <<endl;
   ...
   cout<< "Filled " << otree->GetEntries() << " entries" <<endl;
...
void peakchan(const char* ifname="tb2012_run_24.root.pulse.root"
      , Int_t ientry1=0
      , Int_t ientry2=-1
      , bool debug=false
      , const char* cfgname="peakchan.cfg"
      )
{
   ...
   if (!itree) {
      cout<< "Could not find tree 'p' in the file " << ifname <<endl;
      return;
   }

   TFile* ofile = TFile::Open(Form("%s.peaks.root",ifname), "recreate");

   peakchan(itree, ientry1, ientry2, debug, cfgname);

   cout<< "Output file " << ofile->GetName() <<endl;
   ofile->Write();
}
...
Peak* getpeak(Int_t evt, TTree* speaks)
{
   ...
   if (speaks->LoadTree(evt) < 0) return 0;
